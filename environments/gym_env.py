...
class SimpleMultiAgentEnv(gym.Env):
    def __init__(self, dims: int, n_agents: int = 1, n_landmarks: int = 1, clip=False):
        self.dims = dims
    ...
    def step(self, actions: List[np.ndarray]):
        ...
        rewards = [-np.sum((s - l) ** 2) for s,l in zip(self.states, self.landmarks)]
        dones = [False] * self.n_agents  # currently this environment does not know the final pos
