...
class TFFunctionEnvironment(tf_environment.TFEnvironment):
    def __init__(self, function: Function, dims, clip_actions: bool = False):
        ...
        self._episode_ended = common.create_variable(name='episode_ended', initial_value=False, dtype=tf.bool)
        self._steps_taken = common.create_variable(name='steps_taken', initial_value=0, dtype=tf.int32)
        self._state = common.create_variable(name='state',
                                             initial_value=self._generator.uniform(shape=observation_spec.shape,
                                                                                   minval=self._domain_min,
                                                                                   maxval=self._domain_max,
                                                                                   dtype=tf.float32),
                                             dtype=tf.float32)

        self._last_objective_value = common.create_variable(name='last_objective_value',
    ...
    def _reset(self) -> ts.TimeStep:
        ...
        with tf.control_dependencies([reset_ended, reset_steps]):
            state_reset = self._state.assign(value=self._generator.uniform(shape=self.observation_spec().shape,
                                                                           minval=self._domain_min,
                                                                           maxval=self._domain_max,
                                                                           dtype=tf.float32))
    ...
    def _step(self, action):
        ...
        def take_step():
            ...
            with tf.control_dependencies([new_state]):
                ...
                episode_finished = tf.cond(pred=tf.math.greater_equal(self._steps_taken, MAX_STEPS),
    ...
    def get_info(self, to_numpy=False):
        if to_numpy:
            return FunctionEnvironmentInfo(position=self._last_position.value().numpy(),
