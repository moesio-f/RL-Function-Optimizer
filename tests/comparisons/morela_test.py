...
function = np_functions.Ackley()
dims = 10
max_iterations = 15000
learning_rate = 0.8
discount = 0.2
env_size = 20
beta = rng.uniform(low=function.domain.min, high=function.domain.max, size=dims).astype(np.float32)

original_environment: [TableRow] = []
last_step: TableRow
best_solution: TableRow = TableRow(array=beta.copy(), fitness=np.finfo(np.float32).max)
...
for _ in range(env_size):
    ...
    original_environment.append(TableRow(array=random_vector, fitness=fitness))
...
for t in range(2, max_iterations + 1):
    ...
    # Thus, we must sort original_env_{t} and new_sub_env_{t}
    # Then, we compare both and if worst(original_env_{t}) > best(new_sub_env_{t})
    #       then we delete worst(original_env_{t}) from original_env_{t}
    #       and add best(new_sub_env_{t}) to original_env_{t}
    ...
    for row in original_environment:
        ...
        row.array = (1 - learning_rate) * row.array + learning_rate * (reward + discount * best_solution.array)
